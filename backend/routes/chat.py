...
Keep the summary focused, clear, and easy to review before an exam."""
        ...
        # ── MODE: GENERATE ────────────────────────────────────────────────────
            ...
            # Exam prompts are template-generated by the frontend with only
            # the user-supplied topic and uploaded document content substituted
            # in.  The templates themselves contain "You are [role]…" persona
            # patterns and the document body may contain markup that looks like
            # injection (e.g. "### Instructions", "<context>", "[INST]") — all
            # of which produce false positives in both the regex and the LLM
            # classifier.
            #
            # For exam prompts, skip injection screening entirely.  The
