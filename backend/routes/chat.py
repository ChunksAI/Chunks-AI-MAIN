    ...
    try:
        ...
        if mode == 'visual_tutor':
            vt_system = (
                ...
                "  4. Is the composition visually balanced and centered in the viewBox?\n"
                "  5. Does the content fill at least 75% of the viewBox with no large empty regions?\n"
                "  If any answer is NO, regenerate the SVG once internally before outputting.\n\n"
                "SVG FORMAT:\n"
                "- Choose viewBox orientation based on the subject: use '0 0 300 400' for tall/portrait subjects (human body, brain, buildings, trees), '0 0 400 400' for roughly square subjects, or '0 0 400 300' for wide/landscape subjects (cars, landscapes, horizons).\n"
                "- Add preserveAspectRatio='xMidYMid meet' width='100%' to the root <svg> — do NOT set a fixed height attribute.\n"
                "- FILL THE VIEWBOX: the diagram composition must occupy at least 75% of the viewBox area. Add ~10% padding on all sides so nothing is clipped, but leave no large empty regions.\n"
