...
export async function _wsAsk(question) {
  ...
  wsAppendThinking();
  const capturedSelection = _wsSelectedText;
  _wsSelectedText = '';  // clear so it doesn't bleed into follow-up questions
  try {
...
function _wsOnSelectionChange() {
  ...
  const pdfWrap = document.getElementById('ws-pdf-canvas-wrap');
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('ws-chat-input')?.addEventListener('input', () => {
    if (document.getElementById('ws-chat-input').value.trim()) {
      // User is typing — keep the captured selection but restore placeholder
      document.getElementById('ws-chat-input').placeholder = 'Ask a follow-up about Chapter 3…';
    }
  });
});
...
window.wsChatSend = async function() {
  ...
  const selQuote = _wsSelectedText
    ? `<div style="margin-bottom:7px;padding:7px 10px;border-left:2px solid var(--gold);background:var(--gold-muted);border-radius:0 6px 6px 0;font-size:11px;color:var(--text-3);line-height:1.5;font-style:italic;">"${_wsSelectedText.slice(0,160).replace(/&/g,'&amp;').replace(/</g,'&lt;')}${_wsSelectedText.length>160?'…':''}"</div>`
    : '';
